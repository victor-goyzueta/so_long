...
/*   Updated: 2025/02/28 19:25:56 by vgoyzuet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_load_texture(t_game *game)
{
	if (!game->texture->player || !game->texture->close
		...
		|| !game->texture->wall_ur || !game->texture->enemy)
...
void	load_texture_4(t_game *game)
{
	...
	if (!cur)
		free_exit(EXIT_FAILURE, game, FAIL_ALLOC, NULL);
	set_current(game, PATH_TEXT, "enemy.xpm", &cur);
	game->texture->enemy = mlx_xpm_file_to_image(game->mlx, cur,
			&(game->window->width), &(game->window->height));
	free(cur);
}
