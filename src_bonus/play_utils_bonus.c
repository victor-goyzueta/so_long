...
/*   Updated: 2025/02/28 21:26:53 by vgoyzuet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	move_enemy(t_game *game, int new_x, int new_y)
{
	if (!game)
		free_exit(EXIT_FAILURE, game, FAIL_ALLOC, NULL);
	if (game->map->matrix == '1')
}
...
static void	move_player(t_game *game, int col, int row)
{
	...
	if (game->map->matrix[new_y][new_x] == '1')
		return ;
	else if (game->map->matrix[new_y][new_x] == 'B')
