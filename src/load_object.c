...
/*   Updated: 2025/02/19 17:12:38 by vgoyzuet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_object(t_game *game)
{
	...
	game->texture->player = mlx_xpm_file_to_image(game->mlx, cur, &(game->window->size), &(game->window->size));
	set_current(PATH_TEXT, "exit.xpm", &cur);
	game->texture->exit = mlx_xpm_file_to_image(game->mlx, cur, &(game->window->size), &(game->window->size));
	set_current(PATH_TEXT, "item.xpm", &cur);
	game->texture->item = mlx_xpm_file_to_image(game->mlx, cur, &(game->window->size), &(game->window->size));
	set_current(PATH_TEXT, "wall.xpm", &cur);
	game->texture->wall = mlx_xpm_file_to_image(game->mlx, cur, &(game->window->size), &(game->window->size));
	set_current(PATH_TEXT, "floor.xpm", &cur);
	game->texture->floor = mlx_xpm_file_to_image(game->mlx, cur, &(game->window->size), &(game->window->size));
